...
```shell-session
pepedavidphone@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
pepedavidphone@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
```
...
We saw how to decode a base64 string using Powershell. Now, let's do the reverse operation and encode a file.
...
pepedavidphone@htb[/htb]$ md5sum hosts 
...
```shell-session
pepedavidphone@htb[/htb]$ pip3 install uploadserver
...
![[Pasted image 20250422114657.png]]


```shell-session
pepedavidphone@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
