...
```shell-session
pepedavidphone@htb[/htb]$ cat /etc/passwd
...
# Windows Authentication Process
  ...
  ![[Pasted image 20250508055352.png]]
...
![[Pasted image 20250508055527.png]]Credential Manager is a feature built-in to all Windows operating systems that allows users to save the credentials they use to access various network resources and websites. Saved credentials are stored based on user profiles in each user's `Credential Locker`. Credentials are encrypted and stored at the following location:
